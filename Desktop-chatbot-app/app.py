...
class Chatbot:
    def __init__(self, window):
        window.title('SidTron')
        window.geometry('700x700')
        window.resizable(0, 0)
        window.configure(bg="#121212")
        self.message_session = Text(window, bd=3, relief="flat", font=(
            "Times", 11), undo=True, wrap="word")
        self.message_session.config(
            width=45, height=15, bg="#121212", fg="white", state='disabled')
    ...
    def reply_to_you(self, event=None):
        ...
        if close_match:
            reply = 'SidTron: ' + self.Brain[close_match[0]][0] + '\n'
        else:
            reply = 'SidTron: ' + 'Cant it in my knowledge base\n'
